...
export default function TraitDropdown(props) {
    ...
    return (
        <>
            <label className="capitalize text-xl text-blue-200 place-self-center" htmlFor={`dropdown-${props.type}`}>
                {props.type}
            </label>
            <select onChange={() => props.onTraitChange(event, props.type)} id={`dropdown-${props.type}`} value={selectedTrait ?? "undefined"}
                className="bg-transparent border-2 rounded-xl border-blue-400/70 p-3 text-white">
